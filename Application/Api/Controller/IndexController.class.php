...
class IndexController extends HomeBaseController{
    ...
    public function oauth(){
        ...
        if(is_array($token)){
            ...
            // 获取本地数据库的用户数据
            $user_data=D('User')->getDataByOpenid($data['openid']);
            // 如果登录过 则覆盖；没有登录这添加数据
            if(empty($user_data)){
                $id=D('User')->addData($data);
            }else{
                $id=D('User')->editData($data);
            }
            // 组合存session的数据

            $login_info=D('User')->where(array('user_id'=>$id))->find();
            session('ycblog1224userinfo',$login_info);
            // 跳转到登录前的页面
